...
int32_t MCI_Init(uint8_t powerActiveLevel )
{
   ...
   for ( i = 0; i < 0x100000; i++ );
	...
	//SD_CLK
	PINSEL_SetPinFunc(1, 2, 2);

	//SD_CMD
	PINSEL_SetPinFunc(1, 3, 2);

	//SD_PWR
	PINSEL_SetPinFunc(1, 5, 2);

	//SD_DAT_0
	PINSEL_SetPinFunc(1, 6, 2);

	//SD_DAT_1
	PINSEL_SetPinFunc(1, 7, 2);

	//SD_DAT_2
	PINSEL_SetPinFunc(1, 11, 2);

	//SD_DAT_3
	PINSEL_SetPinFunc(1, 12, 2);
